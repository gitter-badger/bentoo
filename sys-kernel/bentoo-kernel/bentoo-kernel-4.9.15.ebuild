...
EAPI="6"
inherit bentoo-kernel

DESCRIPTION="Bentoo Linux kernel"
KEYWORDS="amd64 arm64"

RDEPEND="=sys-kernel/bentoo-modules-${PVR}"
DEPEND="${RDEPEND}
	app-arch/gzip
	app-shells/bash
	sys-apps/coreutils
	...
	sys-fs/mdadm
	sys-fs/xfsprogs
	>=sys-kernel/bentoo-firmware-20160331-r1:=
	sys-kernel/dracut
	virtual/udev"
...
#	fi
# /usr/bin/dracut --force --no-hostonly --no-compress --omit i18n --omit lvm --omit network --omit terminfo --kver [15/130]
entoo /usr/share/bootengine/bootengine.cpio
}
...
src_install() {
	# coreos-postinst expects to find the kernel in /boot
	insinto "/boot"
	newins "$(kernel_path)" "vmlinuz-${KV_FULL}"
	dosym "vmlinuz-${KV_FULL}" "/boot/vmlinuz"

	insinto "/lib/modules/${KV_FULL}/build"
