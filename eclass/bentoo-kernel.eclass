...
# Use source installed by bentoo-sources
KERNEL_DIR="${SYSROOT}/usr/src/${BENTOO_SOURCE_NAME}"
...
install_build_source() {
	...
	rm "${D}/lib/modules/${KV_FULL}"/{build,source} || die
	...
	# Install a stripped source for out-of-tree module builds (Debian-derived)
	{
		...
		--owner=root:root \
		--dereference \
		"${D}/lib/modules/${KV_FULL}" || die
}

bentoo-kernel_pkg_pretend() {
